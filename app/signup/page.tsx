 ...
 *  a string rather than a character class so no escape can go quietly wrong. */
const SYMBOLS = '^$*.[]{}()?"!@#%&/\\,><\':;|_~`+=- ';

/* Cognito's default policy, in the order people usually satisfy it. */
const RULES = [
  ...
  { id: 'number', label: 'Number', test: (v: string) => /[0-9]/.test(v) },
  {
    id: 'symbol',
    label: 'Symbol',
    test: (v: string) => Array.from(v).some((ch) => SYMBOLS.includes(ch)),
  },
] as const;

const STEP_WORDS = ['Account', 'Confirm'] as const;
...
/* ------------------------------------------------------------------ */
/* Scoped stylesheet — shared verbatim with app/login/page.tsx so that   */
...
export default function SignupPage() {
  ...
  const passwordReady = met.every(Boolean);
  const canCreate =
    name.trim().length > 0 && EMAIL_PATTERN.test(email.trim()) && passwordReady;

  /* ---------------- create account ---------------- */

  const finishWithLogin = useCallback(async () => {
    /* The password is still in state, so confirmed users go straight in
       rather than retyping it one screen later. */
    try {
      ...
      if (res.ok && data.ok === true) {
        if (alive.current) setRedirecting(true);
        /* A full reload, not `router.push`: the app state provider lives in
           ...
           had cached — the previous account's profile, on a device that was
           signed in as someone else — straight past onboarding. */
    ...
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        if (data.needsConfirmation === false) {
          const wentIn = await finishWithLogin();
          if (wentIn || !alive.current) return;
          router.push('/login');
          return;
        }
        ...
        if (alive.current) {
          setError("Couldn't reach the sign-up service. Try again in a moment.");
        }
      } finally {
  ...
  const handleConfirm = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        if (!res.ok || data.ok !== true) {
          setError(text(data.error) || "That code didn't work. Try again.");
    ...
    [code, email, finishWithLogin, redirecting, router, submitting],
  );
  ...
  const handleResend = useCallback(async () => {
    ...
    try {
      ...
      if (alive.current) setError("Couldn't reach the sign-up service.");
    } finally {
          ...
          <>
            ...
            >
              {STEP_WORDS.map((word, i) => (
                <Fragment key={word}>
                  {i > 0 ? <span className="ya-tick" /> : null}
                  <span className="ya-step" data-state={stepState(i)}>
                    {word}
                  </span>
                </Fragment>
              ))}
            ...
            <p style={srOnly} role="status" aria-live="polite">
              {`Step ${stepIndex + 1} of 2: ${STEP_WORDS[stepIndex]}`}
            </p>
          </>
        ) : null}
      </header>
        ...
        <div className="ya-stage" data-phase={phase} key={step}>
          ...
          {!configured ? (
            ...
            <>
              ...
              <h1 className="ya-h1">
                Take a seat. <em>Then tell us who you are.</em>
              </h1>
              <p className="ya-sub">
                A name, an email, a password — that&rsquo;s the whole form. The
                part that actually matters comes next.
              </p>

              <form className="ya-form" onSubmit={handleCreate} noValidate>
                ...
                >
                  {RULES.map((rule, i) => (
                    <li key={rule.id} className="ya-rule" data-met={met[i]}>
                      <span className="ya-rule-mark" aria-hidden="true" />
                      {rule.label}
                      <span style={srOnly}>
                        {met[i] ? ' — done' : ' — still needed'}
                      </span>
                    </li>
                  ))}
                </ul>
                ...
                <button
                  type="submit"
                  className="ya-cta"
                  disabled={!canCreate || submitting || redirecting}
                >
                  {redirecting
                    ? 'Opening Yellow…'
                    : submitting
              ...
              <p className="ya-footnote">
                ...
                <Link href="/login" className="ya-link">
                  Sign in
                </Link>
              </p>
            ...
            <>
              ...
              <h1 className="ya-h1">
                We sent you a <em>six-digit code</em>.
              </h1>
              <p className="ya-sub">
                It&rsquo;s on its way to <b>{email.trim().toLowerCase()}</b>.
